...
- Both server machine and iPad on the same network (or connected via Tailscale)
...
### 2. Set environment and start the server
...
```bash
tailscale ip -4    # get server's Tailscale IP
# Open Safari on iPad → http://<ip>:<port>
```
